...
class LstmModel(nn.Module):
    #TODO: try setting bach size manually
...
##
t0=time.time()
model_event_1 = LstmModel(input_dim=356, hidden_dim=30, num_layers=2).to(device)
dataset_event_1 = DatasetEventBased(file_paths=list(glob.glob('data/lmd_full/**/*.mid', recursive=True))[:3000], seq_len=SEQ_LEN*2)
print(time.time()-t0)
print('len', len(dataset_event_1))

t0=time.time()
train(dataset_event_1, model_event_1, 2, nn.CrossEntropyLoss())
print(time.time()-t0)
model_path = 'models/big_event_3000.pth'
torch.save(model_event_1.state_dict(), model_path)
...
##
model_note = LstmModel(input_dim=2).to(device)
model_note.load_state_dict(torch.load('models/big_note_5.pth'))

##
model_note = torch.load('models/big_note_5.pth')
...
##
#TODO: upload model to tensorboard
...
[torch.argmax(oh).item() for oh in seq_event_based]
##
